...
function main {
    ...
    do
        # cache
        python tools/run_net.py --cfg ${CONFIG_DIR} --dataset_dir ${DATASET_DIR} \
            --batch_size 1 \
        ...
        do
            if [ $batch_size -le 0 ];then
                if [ "${model_name}" == "MViT_B" ];then
                    batch_size=16
                elif [ "${model_name}" == "MVITv2_S" ];then
                    batch_size=16
                else
                    batch_size=64
                fi
...
function generate_core_launcher {
    ...
    do
        ...
        printf "python -m oob-common.launch --enable_jemalloc \
            ...
            tools/run_net.py --cfg ${CONFIG_DIR} --dataset_dir ${DATASET_DIR} \
                --batch_size $batch_size \
                --num_iter $num_iter --num_warmup $num_warmup \
                --precision $precision \
                --channels_last $channels_last \
